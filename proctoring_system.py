"""
Proctoring System - Core Module
================================
Face recognition, gaze tracking, audio monitoring, and rule engine
for exam proctoring. Designed to work both as standalone application
and as a backend module for FastAPI web application.
"""
...
class Thresholds:
    """Centralized thresholds for all detection modules"""
...
@dataclass
class Alert:
    """Represents a single rule violation alert"""
    rule_name: str
    severity: str
    message: str
    timestamp: str
...
def rule_identity_confidence(record: dict, state: SessionState) -> Optional[Alert]:
    """Check if face identity confidence is too low"""
...
def rule_gaze_away(record: dict, state: SessionState) -> Optional[Alert]:
    """Check for sustained gaze away from screen"""
    gaze = record.get("gaze", {})
    
    # Handle both dictionary and object gaze data
    if isinstance(gaze, dict):
        ...
        flag_away = False
        left_dx = left_dy = right_dx = right_dy = 0.0

    numeric_away = (
        (left_dx >= Thresholds.GAZE_DX_THRESHOLD or left_dy >= Thresholds.GAZE_DY_THRESHOLD)
        and (right_dx >= Thresholds.GAZE_DX_THRESHOLD or right_dy >= Thresholds.GAZE_DY_THRESHOLD)
    )
...
def rule_unauthorized_object(record: dict, state: SessionState) -> Optional[Alert]:
    """Check for unauthorized objects in frame"""
...
_registry_tensor: Optional[torch.Tensor] = None
...
def _embed(pil_img: Image.Image) -> Optional[torch.Tensor]:
...
def get_embedding_from_path(img_path: str) -> Optional[torch.Tensor]:
    """Get face embedding from image file path"""
    if not os.path.exists(img_path):
        return None
    return _embed(Image.open(img_path).convert("RGB"))


def get_embedding_from_frame(frame_bgr: np.ndarray) -> Optional[torch.Tensor]:
    """Get face embedding from BGR numpy frame"""
    rgb_frame = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
    return _embed(Image.fromarray(rgb_frame))


def get_embedding_from_frame_bytes(frame_bytes: bytes) -> Optional[torch.Tensor]:
    """Get face embedding from JPEG/PNG bytes"""
    nparr = np.frombuffer(frame_bytes, np.uint8)
    img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
    if img is None:
        return None
    return get_embedding_from_frame(img)
...
def register_person(name: str, img_paths: list) -> dict:
    ...
    _rebuild_registry_tensor()
    logger.info(f"✓ Registered: {name} ({len(embeddings)} photos)")
...
def _match_embedding(emb: torch.Tensor) -> dict:
    """Match embedding against registry"""
...
def identify_person_from_frame(frame_bgr: np.ndarray) -> dict:
    """Identify person from BGR frame"""
    emb = get_embedding_from_frame(frame_bgr)
    return {"error": "no face detected"} if emb is None else _match_embedding(emb)


def identify_person_from_frame_bytes(frame_bytes: bytes) -> dict:
    """Identify person from JPEG/PNG bytes"""
    emb = get_embedding_from_frame_bytes(frame_bytes)
    return {"error": "no face detected"} if emb is None else _match_embedding(emb)
...
class GazeTracker:
    ...
    def _gaze_dir(self, iris_center: np.ndarray, eye_lms: list, 
                  fw: int, fh: int, lm) -> Tuple[str, str, float, float]:
    ...
    def _process_eye(self, iris_ids: list, eye_ids: list, smooth: dict,
                     fw: int, fh: int, lm) -> dict:
        """Process single eye gaze"""
    ...
    def process(self, frame: np.ndarray) -> dict:
        """Process frame and return gaze data"""
...
class AudioMonitor:
    ...
    def start(self):
        ...
        if sd is None:
            logger.warning("Audio disabled in cloud environment")
            return
...
class ObjectDetector:
    """Detect unauthorized objects in frame using YOLO"""
    UNAUTHORIZED = {
        63: "Laptop", 64: "Laptop",
...
class ProctoringSystem:
    """Integrated proctoring system with webcam, audio, and detection modules"""
    ...
    def initialize(self):
        """Initialize all detection modules and hardware"""
        _s = sys.stderr
        sys.stderr = _Devnull()
        try:
            try:
            ...
            try:
                ...
                logger.warning("Face detection model not available, using MTCNN only")
                self.yolo_face_model = None
    ...
    def process_frame(self, frame: np.ndarray) -> dict:
        """Process a single frame for API use"""
        self.frame_count += 1
    ...
    def _run_detections(self, frame: np.ndarray) -> np.ndarray:
        """Run face and object detection, draw boxes"""
        self.face_count = 0
        if self.yolo_face_model:
            for res in self.yolo_face_model(frame, conf=0.5, verbose=False):
                if res.boxes is None:
    ...
    def _run_recognition(self, frame: np.ndarray) -> np.ndarray:
        """Run face recognition and draw label"""
        if self.frame_count % self.identify_every_n == 0:
            result = identify_person_from_frame(frame)
            if "best_match" in result:
                self.current_identity = result
    ...
    def save_detections(self, filename: str = None) -> str:
        """Save detection data and rule report to disk"""
        if filename is None:
    ...
    def cleanup(self):
        ...
        logger.info("Cleaning up proctoring system resources...")
        ...
        if hasattr(self, 'gaze_tracker') and self.gaze_tracker:
...
if __name__ == "__main__":
    ...
    system = None
    try:
        system = ProctoringSystem(headless=False)
        system.initialize()
        system.run()
